...
void main() {
  group('BinaryOPs', () {
    test('add, subtract, multiply, divide, pow', () {
      ...
      expect(op.add(a, b).readMap<mnn.float32>().value, closeTo(12.0, 0.001));
      expect(op.subtract(a, b).readMap<mnn.float32>().value, closeTo(8.0, 0.001));
      expect(op.multiply(a, b).readMap<mnn.float32>().value, closeTo(20.0, 0.001));
      expect(op.divide(a, b).readMap<mnn.float32>().value, closeTo(5.0, 0.001));
      expect(op.pow(a, b).readMap<mnn.float32>().value, closeTo(100.0, 0.001));
    ...
    test('minimum, maximum', () {
      ...
      expect(op.minimum(a, b).readMap<mnn.float32>().value, closeTo(2.0, 0.001));
      expect(op.maximum(a, b).readMap<mnn.float32>().value, closeTo(10.0, 0.001));
    ...
    test('biasAdd, squaredDifference', () {
      ...
      expect(op.biasAdd(a, b).readMap<mnn.float32>().value, closeTo(12.0, 0.001));
      expect(op.squaredDifference(a, b).readMap<mnn.float32>().value, closeTo(64.0, 0.001)); // (10-2)^2
    ...
    test('floorDiv, floorMod', () {
      ...
      expect(op.floorDiv(a, b).readMap<mnn.float32>().value, closeTo(5.0, 0.001));
      expect(op.floorMod(a, b).readMap<mnn.float32>().value, closeTo(0.5, 0.001));
    ...
    test('GridSample, CosineSimilarity', () {
      ...
      expect(cs.readMap<mnn.float32>().value, closeTo(1.0, 0.001));
      cs.dispose();
      x1.dispose();
      x2.dispose();
      dim.dispose();
  ...
  group('UnaryOPs', () {
    test('abs, negative, sign', () {
      ...
      expect(op.abs(a).readMap<mnn.float32>().value, closeTo(5.0, 0.001));
      expect(op.negative(a).readMap<mnn.float32>().value, closeTo(5.0, 0.001));
      expect(op.sign(a).readMap<mnn.float32>().value, closeTo(-1.0, 0.001));
    ...
    test('floor, ceil, round', () {
      ...
      expect(op.floor(a).readMap<mnn.float32>().value, closeTo(3.0, 0.001));
      expect(op.ceil(a).readMap<mnn.float32>().value, closeTo(4.0, 0.001));
      expect(op.round(a).readMap<mnn.float32>().value, closeTo(4.0, 0.001));
    ...
    test('square, sqrt, rsqrt', () {
      ...
      expect(op.square(a).readMap<mnn.float32>().value, closeTo(16.0, 0.001));
      expect(op.sqrt(a).readMap<mnn.float32>().value, closeTo(2.0, 0.001));
      expect(op.rsqrt(a).readMap<mnn.float32>().value, closeTo(0.5, 0.001));
    ...
    test('trig functions', () {
      ...
      expect(op.sin(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.cos(a).readMap<mnn.float32>().value, closeTo(1.0, 0.001));
      expect(op.tan(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      a.dispose();
    });
    ...
    test('exp, log, reciprocal', () {
      ...
      expect(op.exp(a).readMap<mnn.float32>().value, closeTo(2.718, 0.001));
      expect(op.log(op.exp(a)).readMap<mnn.float32>().value, closeTo(1.0, 0.001));

      final b = mnn.VARP.scalar<mnn.float32>(2.0);
      expect(op.reciprocal(b).readMap<mnn.float32>().value, closeTo(0.5, 0.001));
    ...
    test('hyperbolic functions', () {
      final a = mnn.VARP.scalar<mnn.float32>(0.0);
      expect(op.sinh(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.cosh(a).readMap<mnn.float32>().value, closeTo(1.0, 0.001));

      // tanh is already tested elsewhere but adding here for completeness
      expect(op.tanh(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
    ...
    test('inverse trig/hyperbolic', () {
      ...
      expect(op.asin(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.atan(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.asinh(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.atanh(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));

      final b = mnn.VARP.scalar<mnn.float32>(1.0);
      expect(op.acos(b).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.acosh(b).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
    ...
    test('erf, erfc, erfinv', () {
      final a = mnn.VARP.scalar<mnn.float32>(0.0);
      expect(op.erf(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
      expect(op.erfc(a).readMap<mnn.float32>().value, closeTo(1.0, 0.001));

      // erfinv(0) = 0
      expect(op.erfinv(a).readMap<mnn.float32>().value, closeTo(0.0, 0.001));
  ...
  group('ReduceOPs', () {
    test('reduceSum, reduceMean, reduceMax, reduceMin, reduceVariance', () {
      ...
      expect(mean.readMap<mnn.float32>().value, closeTo(2.5, 0.001));

      final max = op.reduceMax(x, axis: [0, 1]);
      expect(max.readMap<mnn.float32>().value, closeTo(4.0, 0.001));

      final min = op.reduceMin(x, axis: [0, 1]);
      expect(min.readMap<mnn.float32>().value, closeTo(1.0, 0.001));

      final variance = op.reduceVariance(x, axis: [0, 1]);
      expect(variance.readMap<mnn.float32>().value, closeTo(1.25, 0.001));
    ...
    test('reduceProd', () {
      ...
      expect(prod.readMap<mnn.float32>().value, closeTo(24.0, 0.001));
  ...
  group('OtherOPs', () {
    ...
    test('OtherOps Bulk 1', () {
      ...
      expect(norm.readMap<mnn.float32>().value, isNotNull);
      norm.dispose();
  ...
  group('Neural Network Ops', () {
    test('NN Bulk 1', () {
      ...
      expect(sc.readMap<mnn.float32>().value, closeTo(2.0, 0.001));
      sc.dispose();

      // ReLU6, PReLU, softMax, softPlus, softSign
      expect(op.ReLU6(x).readMap<mnn.float32>().value, isNotNull);
      expect(op.PReLU(x, [0.1]).readMap<mnn.float32>().value, isNotNull);
      expect(op.softMax(x).readMap<mnn.float32>().value, isNotNull);
      expect(op.softPlus(x).readMap<mnn.float32>().value, isNotNull);
      expect(op.softSign(x).readMap<mnn.float32>().value, isNotNull);
    ...
    test('NN Bulk 2', () {
      ...
      expect(op.convert(x, mnn.DimensionFormat.NC4HW4).readMap<mnn.float32>().value, closeTo(1.0, 0.001));
      // transpose1 takes VARP perm
      final perm = mnn.VARP.list<mnn.int32>([0, 1, 3, 2]);
      expect(op.transpose1(x, perm).dim, [1, 1, 2, 2]);
      perm.dispose();

      expect(op.channelShuffle(x, 1).readMap<mnn.float32>().value, closeTo(1.0, 0.001));
      // TODO: fails
    ...
    test('NN Bulk 3', () {
      ...
      // selu, elu, threshold
      expect(op.selu(x, 1.0, 1.0).readMap<mnn.float32>().value, isNotNull);
      expect(op.elu(x).readMap<mnn.float32>().value, isNotNull);
      expect(op.threshold(x).readMap<mnn.float32>().value, isNotNull);
      ...
      expect(i8.readMap<mnn.int8>().value, isNotNull);
      final f = op.int8ToFloat(i8, scale);
      expect(f.readMap<mnn.float32>().value, isNotNull);
      i8.dispose();
      f.dispose();
      scale.dispose();
    ...
    test('avgPool', () {
      ...
      expect(output.readMap<mnn.float32>().value, closeTo(2.5, 0.001));
  ...
  group('More Ops', () {
    test('activations: gelu, sigmoid, silu, hardswish, log1p, expm1', () {
      ...
      expect(op.gelu(a).readMap<mnn.float32>().value, isNotNull);
      expect(op.sigmoid(a).readMap<mnn.float32>().value, closeTo(0.731, 0.001));
      expect(op.silu(a).readMap<mnn.float32>().value, isNotNull);
      expect(op.hardswish(a).readMap<mnn.float32>().value, isNotNull);
      expect(op.log1p(a).readMap<mnn.float32>().value, closeTo(0.693, 0.001));
      expect(op.expm1(a).readMap<mnn.float32>().value, closeTo(1.718, 0.001));
    ...
    test('math: atan2, mod', () {
      ...
      expect(op.atan2(y, x).readMap<mnn.float32>().value, closeTo(0.785, 0.001)); // pi/4

      final m1 = mnn.VARP.scalar<mnn.float32>(5.0);
      final m2 = mnn.VARP.scalar<mnn.float32>(3.0);
      expect(op.mod(m1, m2).readMap<mnn.float32>().value, closeTo(2.0, 0.001));
    ...
    test('manipulation: stack, unstack, tile, broadcastTo', () {
      ...
      expect(u.length, 2);
      expect(u[0].readMap<mnn.float32>().value, 1.0);
