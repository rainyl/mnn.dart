...
void testSession(mnn.Session session) {
...
Future<void> testInferenceMnistCopy(
  mnn.Session session, {
  ...
  expect(input.copyFromHost(inputTensor), true);
  inputTensor.dispose();
...
Future<void> testInferenceMnistMapping(
  mnn.Session session, {
  String imgPath = "test/data/mnist_0.png",
  int target = 0,
  bool runAsync = false,
...
void main() async {
  group('Interpreter', () {
    ...
    test('session', () async {
      ...
      testSession(session);
      await testInferenceMnistCopy(session);
    });
    ...
    test('runtime', () async {
      ...
      await testInferenceMnistCopy(session1);
      await testInferenceMnistCopy(session2);
    });
  });

  for (final runAsync in [true, false]) {
    test('inference with copy', () async {
      final model = mnn.Interpreter.fromFile(modelPath);
      expect(model.isEmpty, false);
      final session = model.createSession();
  ...
  for (final runAsync in [true, false]) {
    test('inference with mapping', () async {
      final model = mnn.Interpreter.fromFile(modelPath);
      expect(model.isEmpty, false);
      final session = model.createSession();
      for (final (imgPath, target) in imagePaths) {
